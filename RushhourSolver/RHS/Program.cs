...
namespace RushHourSolver
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Keep track of whether the correct solution has been found
            bool found = false;
            ...
            while (q.Count > 0 && !found)
            {
                ...
                Parallel.For(0, q.Count, index =>
                {
                    Tuple<byte[], Solution> cur;
                    
                    if (q.TryDequeue(out cur))
                    {
                        foreach(Tuple<byte[], Solution> next in Sucessors(cur)){
